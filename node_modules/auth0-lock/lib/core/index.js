...
var _dataFns3 = (0, _data_utils.dataFns)(["core", "auth"]),
    getAuthAttribute = _dataFns3.get;
...
function extractTenantBaseUrlOption(opts, domain) {
  ...
  if ((0, _string_utils.endsWith)(hostname, DOT_AUTH0_DOT_COM)) {
    var parts = hostname.split(".");
    var _domain = parts.length > 3 ? "https://cdn." + parts[parts.length - 3] + DOT_AUTH0_DOT_COM : AUTH0_US_CDN_URL;
    var tenant_name = parts[0];

    return _domain + '/tenants/v1/' + tenant_name;
  } else {
    return domainUrl;
  }
}
